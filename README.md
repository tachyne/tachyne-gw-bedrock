...
1.26.50 / protocol 2193 via gophertunnel): terminates Bedrock clients
...
GeyserMC/mappings (pinned at Geyser's Bedrock 1.26.50 update; its Java side is
matched onto ours by block name and properties), Geyser core's bedrock/
resources (1.26.50) and vanilla's own reports for the Java side — MIT-licensed
data consumed as facts, algorithms reimplemented. The generator refuses to
write a block state or item the target Bedrock version's palette and item
registry do not contain.
